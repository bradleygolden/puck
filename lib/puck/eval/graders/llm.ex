defmodule Puck.Eval.Graders.LLM do
  ...
  defp build_prompt(rubric, output) do
    ...
    Respond with exactly "PASS" if all criteria are met, or "FAIL: <reason>" explaining what criteria were not met.
    """
  end

  defp parse_judgment(content) do
    ...
    cond do
      ...
      String.starts_with?(normalized, "FAIL:") ->
        reason = String.trim_leading(normalized, "FAIL:")
        {:fail, String.trim(reason)}

      true ->
        {:fail, "Invalid judge response: #{content}"}
    end
  end
